...
int get_grammar_id(char* grammar_id, unsigned int id_len)
{
	...
	fp = fopen("/home/ubuntu/catkin_dada/src/ros_xf/src/gm_continuous_digit.abnf", "rb");
	if (NULL == fp)
	{   
...
{	
	...
	ros::NodeHandle n;
	ros::Publisher my_pub = n.advertise<std_msgs::Int8>("my_topic",100);
	
	ros::Rate loop_rate(1);
	int			ret						=	MSP_SUCCESS;
	const char* login_params			=	"appid = 58ae51d7, work_dir = ."; //登录参数,appid与msc库绑定,请勿随意改动
	/*
	* sub:             请求业务类型
	* result_type:     识别结果格式
	...
	*/
	...
	printf("上传语法成功\n");
 	...
 	run_asr("/home/ubuntu/catkin_dada/src/ros_xf/src/wav/lib.wav", session_begin_params, grammar_id); //iflytek01对应的音频内容：“18012345678”
